...
function converge_profile_fixed_itf!(model,ρ,T,z;damping=0.05,interface_loc=0.0)
    ρl =[ρ[i].boundary_conditions[2] for i in @comps]
    Vl = 1/sum(ρl)
    X = ρl./sum(ρl)
    μ_res = Clapeyron.VT_chemical_potential_res(model,Vl,T,X)/R̄/T
    
    if minimum(z) > interface_loc || interface_loc > maximum(z)
        error("Interface index is outside of the bounds.")
    end

    counter = 0
    counter_max = 10
    println("counter_max = ", counter_max)
    ...
    function obj(model,ln_G,ρ,T,z,ln_x,μ_res,ρl,α)
        ...
        for i in comps
            ...
            if counter == counter_max
                z_gds = _find_z_gibbs_dividing_surface(ρ_new, z)
                shift_profile!(ρ[i], interface_loc-z_gds)
                println("shifted profile by ", interface_loc-z_gds)
            end
            if i == comps[end]
                counter += 1
                if counter > counter_max
