...
function ElectrolyteDFTSystem(model::ElectrolyteModel, structure::DFTStructure, options::DFTOptions = DFTOptions())
    ...
    fields = get_fields(model.neutralmodel, species, structure)
    fields_ion = get_fields(model.ionmodel, ion_species, structure)
    ...
    external_field = ElectrostaticPotential(model, structure)

    NF = compute_field_len(fields,dimension(structure))
    chunksize = Val{NF}()
    return ElectrolyteDFTSystem(model, species, ion_species, structure, typed_fields, propagator, external_field,options,chunksize)
end
