...
"""
function f_hs(system::DFTSystem, model::SAFTModel, n, n₃, nᵥnᵥ)
    species = system.species
    m = model.params.segment.values
    HSd = species.size

    n₀ = zero(first(n) + first(m) + first(HSd))
    n₁,n₂,nᵥ₁nᵥ₂,nᵥ₂nᵥ₂,n₃₃ = zero(n₀), zero(n₀), zero(n₀), zero(n₀), zero(n₀)
    for i in 1:length(n)
        mᵢ,HSdᵢ,nᵥnᵥᵢ = m[i],HSd[i],nᵥnᵥ[i]
        nᵢmᵢ = n[i]*mᵢ
        n₀ += nᵢmᵢ/HSdᵢ
        n₁ += 0.5nᵢmᵢ
        n₂ += π*nᵢmᵢ*HSdᵢ
        nᵥ₁nᵥ₂ += -2π*nᵥnᵥᵢ*mᵢ^2/HSdᵢ
