...
# Number of chains (canonical): ρ0 = (n_chains · N + n_molecules) / V
V_approx = Lx * Ly
n_chains = float(round(rho0 * V_approx * f_polymer / N_seg))
n_molecules = float(round(rho0 * V_approx * f_solvent))

# recompute Lx and Ly based on n_chains (square box)
Lx = sqrt((n_chains * N_seg + n_molecules) / rho0)
Ly = Lx

println("=== 2D Asymmetric Diblock Copolymer SCFT (Cylinder Phase) ===")
println("N=$N_seg")
println("χ=$chi_val, κ=$kappa, ρ₀=$rho0")
println("Box: Lx=$(round(Lx; sigdigits=4)) × Ly=$(round(Ly; sigdigits=4))")
println("Grid: $(ngx) × $(ngy), n_chains=$(round(n_chains; sigdigits=4)), n_molecules=$(round(n_molecules; sigdigits=4))")
