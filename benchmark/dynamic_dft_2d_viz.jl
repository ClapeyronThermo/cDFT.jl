...
densities, times = h5open("traj.h5", "r") do f
...
cols_e = [RGBA(c.r, c.g, c.b, α.^2)
        for (c,α) in zip(cgrad([:white,:red],256),
                         range(0,1,length=256))]
anim = Animation()
anim = @animate for n=1:1:length(densities)
    # t=t+dt
    # if mod(n,40)==0
        # x = 0:0.1:2π
        # y = sin.(x .+ (n/2))
        ...
        heatmap(z[:,:,1],c=cols_w, clim=(0,maximum(densities[end][:,:,1])), aspect_ratio=:equal, xaxis=false, yaxis=false, ticks=nothing, interpolate = true)
